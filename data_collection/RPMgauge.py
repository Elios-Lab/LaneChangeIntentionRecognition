...
class RPMGauge:
    def __init__(self, width, height, position=(500, 400)):
        pygame.init()
        self.width = width
        self.height = height
    ...
    def draw_filled_polygon(self, background_width, background_height, outline_pen_width=0):
        if self.scale_polygon_colors is not None:
            ...
            gradient_colors = [(208, 1, 1), (208, 1, 1), (208, 1, 1),]
    ...
    def create_scale_marker_values_text(self):
        ...
        scale_range = self.maxValue - self.minValue
        scale_per_div = scale_range / float(self.scalaCount)  # Calculate scale per division
    ...
    def draw_big_needle_center_point(self):
        ...
        center_x = self.position[0] + self.width / 2
        center_y = self.position[1] + self.height / 2
        diameter = 10  # Adjust the diameter of the center point as desired
    ...
    def update_rpm(self, rpm):
        self.rpm = max(self.minValue * 1000, min(self.maxValue * 1000, rpm))
    ...
    def draw_needle(self, scale_factor=2.5):
        ...
        center_x = self.position[0] + self.width / 2
        center_y = self.position[1] + self.height / 2
        angle = ((self.rpm - self.value_offset - self.minValue * 1000) * self.scale_angle_size /
                    (self.maxValue * 1000 - self.minValue * 1000)) + self.scale_angle_start_value
        ...
        start_pos = [
            center_x + ((needle_length - 147) * math.cos(math.radians(angle))),
            center_y + ((needle_length - 147) * math.sin(math.radians(angle)))
        ]
        end_pos = [
            center_x + ((needle_length - 1)* math.cos(math.radians(angle))),
            center_y + ((needle_length - 1)* math.sin(math.radians(angle)))
        ]
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_UP:
                        self.value = min(self.maxValue, self.value + 1)  # Adjusted for 1 to 8 range
                        self.update_rpm(self.value)
                    elif event.key == pygame.K_DOWN:
                        self.value = max(self.minValue, self.value - 1)  # Adjusted for 1 to 8 range
                        self.update_rpm(self.value)
